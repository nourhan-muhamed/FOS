...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address){
	int myKernPerms = PERM_PRESENT | PERM_USER | PERM_WRITEABLE;
	struct Env* myenv = curenv; //The calling environment
	int checkIfExists = get_share_object_ID(ownerID, shareName);
	if (checkIfExists > -1)
		return E_SHARED_MEM_EXISTS;
	struct Share* newAllocatedSharedObj = NULL;
	int32 myAllocatedSharedObjID = allocate_share_object(&newAllocatedSharedObj);
	if(myAllocatedSharedObjID == E_NO_SHARE)
		return E_NO_SHARE;
	uint32 numberOfPages = ROUNDUP(size,4096) / PAGE_SIZE;
	if (isWritable == 1)
			myKernPerms |= PERM_WRITEABLE;
	for(int i = 0 ; i < numberOfPages ; i++){
		struct FrameInfo* newAllocatedFrame = NULL;
		int frameAllocationDetector = allocate_frame(&newAllocatedFrame);
		if(frameAllocationDetector != E_NO_MEM){
			uint32 currentSVA = (uint32)virtual_address + (i*4096);
			map_frame(myenv->env_page_directory, newAllocatedFrame,currentSVA,myKernPerms);
			add_frame_to_storage(newAllocatedSharedObj->framesStorage,newAllocatedFrame,i);
		}
	}
	strcpy(newAllocatedSharedObj->name, shareName);
	newAllocatedSharedObj->ownerID = ownerID;newAllocatedSharedObj->references = 1;
	newAllocatedSharedObj->size = size;newAllocatedSharedObj->isWritable = isWritable;
	return myAllocatedSharedObjID;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address){
	 int myKernPerms = PERM_PRESENT | PERM_USER;
	struct Env* myenv = curenv; //The calling environment
	int mySharedObjIndex = get_share_object_ID(ownerID, shareName);
	if (mySharedObjIndex == E_SHARED_MEM_NOT_EXISTS)
		return E_SHARED_MEM_NOT_EXISTS;
	struct Share *mySharedObj = &shares[mySharedObjIndex];
	uint32 frameArrSize = ROUNDUP(mySharedObj->size, 4096) / PAGE_SIZE;
	if (mySharedObj->isWritable == 1)
		myKernPerms |= PERM_WRITEABLE;
	for(int i = 0 ; i < frameArrSize ; i++){
		 struct FrameInfo* currentFrame = get_frame_from_storage(mySharedObj->framesStorage, i);
		uint32 currentSVA = (uint32)virtual_address + (i*4096);
		map_frame(myenv->env_page_directory,currentFrame, currentSVA, myKernPerms);
	}
	mySharedObj->references++;
	return mySharedObjIndex;
}
