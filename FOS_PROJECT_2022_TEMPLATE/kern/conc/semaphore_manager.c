...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue){
	...
	if(semID != E_SEMAPHORE_NOT_EXISTS)
	 ...
	 struct Semaphore *mySemObj;
	 int semAllocation = allocate_semaphore_object(&mySemObj);
	 if(semAllocation == E_NO_SEMAPHORE)
		 return E_NO_SEMAPHORE;
	 semaphores[semAllocation].ownerID = ownerEnvID;
	 semaphores[semAllocation].value = initialValue;semaphores[semAllocation].empty = 0;
	 strcpy(semaphores[semAllocation].name, semaphoreName);
	 return semAllocation;
}
// [2] Wait():
void waitSemaphore(int32 ownerEnvID, char* semaphoreName){
	struct Env* envi = curenv; //The calling environment
	int mySemaIndex = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if(mySemaIndex != E_SEMAPHORE_NOT_EXISTS){
		(semaphores[mySemaIndex].value)--;
		if (semaphores[mySemaIndex].value < 0){
			enqueue (&semaphores[mySemaIndex].env_queue, envi);
			envi->env_status = ENV_BLOCKED;
			curenv = NULL;
		}
		fos_scheduler();
	}
